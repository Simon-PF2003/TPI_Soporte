...
def evaluar_linea(mask_movement_color, hsv_roi, x1, y1, x2, y2, lower_color1, upper_color1, lower_color2=None, upper_color2=None, lower_color3=None, upper_color3=None):
    ...
    # Crear máscara de color solo para los píxeles dentro de la línea
    hsv_line = cv2.bitwise_and(hsv_roi, hsv_roi, mask=mask_line)

    # Crear la máscara de color correspondiente para los píxeles en la línea
    mask_line_color = cv2.inRange(hsv_line, np.array(lower_color1), np.array(upper_color1))
    if lower_color2 is not None and upper_color2 is not None:
        mask_line_color2 = cv2.inRange(hsv_line, np.array(lower_color2), np.array(upper_color2))
        mask_line_color = cv2.bitwise_or(mask_line_color, mask_line_color2)
    if lower_color3 is not None and upper_color3 is not None:
        mask_line_color3 = cv2.inRange(hsv_line, np.array(lower_color3), np.array(upper_color3))
        mask_line_color = cv2.bitwise_or(mask_line_color, mask_line_color3)

    # Evaluar cuántos píxeles en la línea tienen el color deseado
    color_in_line = cv2.countNonZero(mask_line_color)

    return color_in_line
    ...
    if len(indexes) > 0:
        for i in indexes.flatten():
            ...
            if movement_detected:
                ...
                # Dentro del bucle, donde trazas la línea y evalúas los píxeles
                for contour in contours:
                    for point in contour:
                        point_x, point_y = point[0][0], point[0][1]
                        # Trazar la línea y evaluar los píxeles tanto para el movimiento como para el color
                        color_in_line = evaluar_linea(mask_movement_color, hsv_roi, point_x, point_y, center_box_x, center_box_y, lower_color1, upper_color1, lower_color2, upper_color2, lower_color3, upper_color3)
                        
                        color_in_movement_pixels += color_in_line

                        # Si hay píxeles del color deseado en la línea, dibujar la línea en la imagen
                        #if color_in_line > 0:
