...
# Inicializar el buffer (basicamente el buffer es uno solo de 5 segundos antes de la deteccion. El post_deteccion va a seguir guardando frames en el video despues de que se detecto al tipo)
fps = int(cap.get(cv2.CAP_PROP_FPS))
post_detection_buffer_size = fps * 10  #10 segundos despues de la deteccion
buffer_size = fps * 5 #5 segundos antes de la deteccion. El video deberia durar un toque mas de 10 segundos porque hay algunos segundos que es cuando detecta.
frame_buffer = deque(maxlen=buffer_size) #Es literalmente el buffer

# Inicializar variables para guardar video
output_video = None #Va a ser donde se guarda el video.
fourcc = cv2.VideoWriter_fourcc(*'mp4v') #Formato de video
frame_size = None
saving_video = False #La uso de bandera nomas. Si se esta guardando un video, guardo los frames actuales
save_counter = 0  # Contador para los segundos después de la detección
...
while cap.isOpened(): #Mientras esté el video abirto e vn leyendo los frames
        ...
        for i in indexes.flatten():
            ...
            if movement_detected:
                ...
                if color_percentage >= 10:
                    color = (0, 0, 255)
                    color_detected = True
                    no_color_detection_counter = 0

                    if output_video is None:
                        # Crear carpeta para guardar videos si no existe
                        ...
                        video_name = f"{color_name}/{timestamp}.mp4"
                        output_video = cv2.VideoWriter(video_name, fourcc, fps, frame_size) #Inicializo el video
                         
                        #Guardo los ultimos frames que tiene el buffer (5 segundos antes de la deteccion)
                        for buffer_frame in frame_buffer:
                            output_video.write(buffer_frame)
                        saving_video = True #Quiero que siga grabando despues de la deteccion
                        save_counter = 0

                    #Guardo frames si esta en True saving video
                    if saving_video:
                        ...
                        #Sigo guardando hasta que no haya capacidad de buffer
                        if save_counter >= post_detection_buffer_size: #Si el buffer no tiene mas capacidad, largo el video nomas
                            saving_video = False
    ...
    if not color_detected and saving_video: #Si no se detecta color y hay un video guardado, guardo un toque mas hasta que se cumpla el tiempo
        save_counter += 1
        frame_buffer.append(frame)
        resized_frame = cv2.resize(frame, frame_size)
        output_video.write(resized_frame)
        print(f"Frames guardados (no color): {save_counter}")
        #Sigo guardando hasta que pasen los 60 segundos
    ...
    # Guardar los frames en el buffer. Basicamente guardo los frames que estan en las iteraciones del loop.
...
if output_video is not None: #Exporto el videito
    output_video.release()