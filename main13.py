...
# Inicializar el buffer circular (15 segundos de buffer)
fps = int(cap.get(cv2.CAP_PROP_FPS))
pre_detection_buffer_size = fps*15 #Son 15 segundos antes de la deteccion
post_detection_buffer_size = fps * 30  #Le puse un toque mas de tiempo despues de la deteccion porque me parecia poco si no
pre_detection_buffer = deque(maxlen=pre_detection_buffer_size) #Son las declaraciones de los buffer
post_detection_buffer = deque(maxlen=post_detection_buffer_size)

#Variables de deteccion. Las uso porque el segundo buffer va a arrancar cuando se detecte la persona
...
while cap.isOpened(): #Mientras esté el video abirto e vn leyendo los frames
    ...
    # Si se detecta una persona, se inicia el buffer de post-detección
    if not detection_occurred:
        pre_detection_buffer.append(frame.copy())
            ...
            if movement_detected:
                ...
                if color_percentage >= 10:
                    ...
                    if output_video is None:
                        ...
                        output_video = cv2.VideoWriter(video_name, fourcc, fps, frame_size)

                        #Guardo los frames de pre_detection_buffer
                        for buffer_frame in pre_detection_buffer:
                ...
                if saving_video:
                    ...
                    if post_detection_counter < post_detection_buffer_size:
                        post_detection_buffer.append(frame.copy())
                    else:
                        saving_video = False
                        detection_occurred = False
